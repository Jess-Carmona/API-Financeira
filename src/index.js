...
function getBalance(statement) {
  const balance = statement.reduce((acc, ops) => {
    if(ops.type === "credit") {
      return acc + ops.amount
    } else {
      return acc - ops.amount;
    }
  }, 0);
...
app.post("/account", (req,res) => {
  ...
  if(customerAlreadyExist) {
    return res.status(400).json({error: "Customer already exsists!"})
  }
...
// other option: app.use(verifyExistsAccountCPF);
